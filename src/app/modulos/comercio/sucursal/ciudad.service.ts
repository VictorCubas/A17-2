...
export class CiudadService extends MetodosServicio<Ciudad> {
    constructor(
        public override http: HttpClient,
        private settings: SettingsService
    ) {
        super(http, 'public/ciudad', Ciudad);
    }

    restartItem(): void {
        this.setItem(new Ciudad());
    }

    /**
     * Método para obtener la ciudad según su id
     *
     * @param ciudadID identificador de la ciudad
     */
    obtenerCiudad(filtro: any, queryParams: any = ''): Observable<any> {
        return this.http.get(
            this.settings.URL_BASE + 'public/ciudad' + queryParams,
            filtro
        );
    }

    crearCiudad(data: any): Observable<any> {
        return this.http.post(this.settings.URL_BASE + 'public/ciudad', data);
    }

    actualizarCiudad(data: any, id: any): Observable<any> {
        return this.http.put(
            this.settings.URL_BASE + 'public/ciudad/' + id,
            data
        );
    }

    eliminarCiudad(id: any): Observable<any> {
        return this.http.delete(this.settings.URL_BASE + 'public/ciudad/' + id);
    }
    ...
    getAll(
        ...
        return this.http.get<BasicResponse<any>>(
            `${this.settings.URL_BASE}/public/pais/departamento/ciudad`,
            { params }
        );
    }
